...
def evaluate_model(model, data_to_evaluate, word2vec, batch_size=64, device='cpu', tuple_input=True):
    model.eval()
    ...
    for (s1_batch, s2_batch) in tqdm(zip(batch_data(sentence1, batch_size), batch_data(sentence2, batch_size)), total=(len(sentence1) // batch_size)):
        ...
        predicted_classes = torch.argmax(output, dim=-1).detach().cpu().numpy()
